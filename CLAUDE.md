...
## ⚠️ זרימת עבודה — הנחיה עליונה, חלה על כל שיחה חדשה
**המשתמש (motitap) הוא מנהל הבוט. הוא עובד מול Claude Code כאן, ומנהל את השרת שלו בנפרד.**

### איך זה עובד:
1. **המשתמש מבקש שינוי** → Claude כותב קוד ועושה **Push ל-main**
2. **השרת של המשתמש מאזין לריפו** → מושך את השינויים אוטומטית (webhook)
3. **המשתמש מפעיל/עוצר/מנטר את הבוט בשרת שלו** → הוא שולט על הכל שם

### כללים מחייבים (אלו לא המלצות — אלו פקודות):
1. **אין לך גישה לשרת — בשום אופן.** אל תנסה ping, curl, ssh, בדיקות רשת, health check, או כל אבחון מרחוק. **זה לא יעבוד ואסור לנסות.**
2. **כשהמשתמש שואל "מה מצב הבוט?" או "תבדוק אם הבוט רץ"** — התשובה היחידה המותרת: **"אין לי גישה לשרת. אתה יכול לבדוק את זה אצלך."** אסור להציע לכתוב סקריפט סטטוס, endpoint, או כל פתרון חלופי אלא אם המשתמש מבקש זאת מפורשות.
3. **אל תשאל שאלות ואל תציע הצעות** — פשוט תעשה מה שהמשתמש מבקש. אם המשתמש אומר "תתקן X" — תתקן. אם הוא אומר "תוסיף Y" — תוסיף. בלי "רוצה שאעשה...?" בלי "אני מציע...".
4. **אחרי כל שינוי קוד → commit → push ל-main → דווח: "הקוד מוכן, ביצעתי Push ל-main כדי שהשרת יתעדכן."**
5. **המשתמש יודע מה הוא עושה.** אל תסביר לו את זרימת העבודה, אל תפרט מה אתה יכול ולא יכול — פשוט תבצע את מה שביקש.
