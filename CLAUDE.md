...
## ⚠️ זרימת עבודה — חובה לקרוא בכל שיחה
המשתמש (motitap) מנהל את הבוט **מהשרת שלו בנפרד**. Claude Code כאן משמש **רק לפיתוח קוד**.
- **אתה כותב קוד, מתקן באגים, ומבצע Push ל-main.**
- **המשתמש מפעיל, עוצר, ומנטר את הבוט בשרת שלו.**
- השרת מאזין לריפו ומושך שינויים אוטומטית אחרי כל Push ל-main.
- **אל תנסה לבדוק אם הבוט רץ, לעשות ping, curl, ssh, או כל בדיקת אבחון רשתית** — אין לך גישה לשרת.
- כשהמשתמש שואל "מה מצב הבוט?" — הוא מנהל את זה בעצמו. תציע לכתוב קוד שיעזור (סקריפט סטטוס, לוגים, וכו'), אל תנסה לבדוק בעצמך.
- **אחרי כל שינוי קוד → Push ל-main → דווח למשתמש שהשרת יתעדכן.**
